...
// The model `parse` (generated by my-http-utils under the `server` feature) is transport-free and
// fails with `HttpParseError`; the server maps it back into the same `HttpFailResult` (status +
// text) the old inline parsing produced. `?` in the generated `handle_request` relies on this.
impl From<my_http_utils::http_input::HttpParseError> for HttpFailResult {
    fn from(e: my_http_utils::http_input::HttpParseError) -> Self {
        use my_http_utils::http_input::HttpParseError::*;
        match e {
            RequiredParameterIsMissing { name, src } => {
                HttpFailResult::required_parameter_is_missing(&name, src)
            }
            CanNotParseValue { name, src, value } => HttpFailResult::invalid_value_to_parse(
                format!("Can not parse '{}' value '{}' from {}", name, value, src),
            ),
            UrlDecodeError(msg) => {
                HttpFailResult::as_fatal_error(format!("Url decode error: {}", msg))
            }
            InvalidBodyFormat(msg) => HttpFailResult::from((412u16, msg)),
